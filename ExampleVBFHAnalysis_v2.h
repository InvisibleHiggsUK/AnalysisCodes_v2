...
// 
//  Specifically dealing with the PU Ntuple
//  University of Bristol
//  Date: September 2014
//
//////////////////////////////////////

#ifndef ExampleVBFHAnalysis_v2_h
#define ExampleVBFHAnalysis_v2_h
...
const Int_t kMaxTrack = 138;
const Int_t kMaxTower = 776;
const Int_t kMaxEFlowTrack = 138;
const Int_t kMaxEFlowPhoton = 430;
const Int_t kMaxEFlowNeutralHadron = 362;
...
public :
   ...
   ExampleVBFHAnalysis_v2(TTree *tree=0);
   virtual ~ExampleVBFHAnalysis_v2();
   virtual Int_t    Cut();
...
#ifdef ExampleVBFHAnalysis_v2_cxx
ExampleVBFHAnalysis_v2::ExampleVBFHAnalysis_v2(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("event_vbf_invH_8TeV_ZTune_PU.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("event_vbf_invH_8TeV_ZTune_PU.root");
      }
...
ExampleVBFHAnalysis_v2::~ExampleVBFHAnalysis_v2()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t ExampleVBFHAnalysis_v2::GetEntry(Long64_t entry)
{
...
Long64_t ExampleVBFHAnalysis_v2::LoadTree(Long64_t entry)
{
...
void ExampleVBFHAnalysis_v2::Init(TTree *tree)
{
...
Bool_t ExampleVBFHAnalysis_v2::Notify()
{
   return kTRUE;
}

void ExampleVBFHAnalysis_v2::Show(Long64_t entry)
{
   if (!fChain) return;
   fChain->Show(entry);
}
Int_t ExampleVBFHAnalysis_v2::Cut()
{
   return 1;
}
#endif 
