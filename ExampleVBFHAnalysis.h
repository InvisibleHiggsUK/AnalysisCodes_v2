...
const Int_t kMaxTrack = 143;
const Int_t kMaxTower = 776;
const Int_t kMaxEFlowTrack = 143;
const Int_t kMaxEFlowTower = 2529;
...
{
  if (tree == 0) {
    TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("event_vbf_invH_8TeV_ZTune_PU_v2.root");
    if (!f || !f->IsOpen()) {
      f = new TFile("event_vbf_invH_8TeV_ZTune_PU_v2.root");
    }
