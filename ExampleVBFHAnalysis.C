...
Float_t nTriggerCut, nEtaCut, nJetPTCut, nDPEtaCut, nDEtaCut, nMETCut, nJetMassCut, nCJVeto, nDPhiCut;
...
void ExampleVBFHAnalysis::processEvents()
{
  ...
  for (Long64_t entry=0; entry<_nEvt;entry++) {       
    ...
    // Print out results
    if(entry == (_nEvt - 1)){
      cout << "nTrigger        : " << nTriggerCut*weight << endl;
      cout << "dijet PT        : " << nYield1*weight << "  " << nYield1/nTriggerCut << "  " << nYield1/nTriggerCut << endl;
      cout << "Eta(j1).Eta(j2) : " << nYield2*weight << "  " << nYield2/nTriggerCut << "  " << nYield2/nYield1 << endl;
      cout << "DeltaEta(j1j2)  : " << nYield3*weight << "  " << nYield3/nTriggerCut << "  " << nYield3/nYield2 << endl;
      cout << "MET             : " << nYield4*weight << "  " << nYield4/nTriggerCut << "  " << nYield4/nYield3 << endl;
      cout << "M(j1j2)         : " << nYield5*weight << "  " << nYield5/nTriggerCut << "  " << nYield5/nYield4 << endl;
      cout << "CJV             : " << nYield6*weight << "  " << nYield6/nTriggerCut << "  " << nYield6/nYield5 << endl;
      cout << "DeltaPhi(j1j2)  : " << nYield7*weight << "  " << nYield7/nTriggerCut << "  " << nYield7/nYield6 << endl;
      ...
      cout << "# Events after Trigger cut : " << nTriggerCut << endl;
      cout << "# Events after DiJet PT cut : " << nJetPTCut << endl;
      cout << "# Events after Eta(j1).Eta(j2) : " << nDPEtaCut << endl;
      cout << "# Events after DeltaEta : " << nDEtaCut << endl;
      cout << "# Events after MET : " << nMETCut << endl;
      cout << "# Events after InvJetMass : " << nJetMassCut << endl;
      cout << "# Events after CJVeto : " << nCJVeto << endl;
      cout << "# Events after DPhiCut : " << nDPhiCut << endl;
    }
  }
}
...
Int_t ExampleVBFHAnalysis::Analysis()
{
  ...
  if(njets.size() > Selection::NJets() ){
    ...
    if(CutTrigger){ nTriggerCut++; }        // Handmade trigger is the first cut of the workflow
    if(Cut0){       nEtaCut++; }
    if(Cut1){       nJetPTCut++; }
    if(Cut2){       nDPEtaCut++; }
    if(Cut3){       nDEtaCut++; }    
    ...
    for(unsigned long _njets = 2; _njets < alljets.size(); _njets++){
      ...
      if(CutTrigger && Cut1 && Cut2 && Cut3 && Cut4 && Cut5 && Cut7 && Selection::CJEta(jets.at(0).Eta(),jets.at(1).Eta(),alljets.at(_njets).Eta()) ){
	_fCJVEt->Fill(alljets.at(_njets).Et()); nCJV++; 
      }
...
Int_t ExampleVBFHAnalysis::Output()
{
  ...
  /*  TFile *_rootFile = new TFile("VBF_inv_8000_unweighted_ZTune_PU_v2.root","RECREATE");
  ...
  */
  ...
  TFile *_rootFile1 = new TFile("VBF_inv_8000_precut_ZTune_PU.root","RECREATE");
